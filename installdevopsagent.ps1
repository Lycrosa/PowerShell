...
[CmdletBinding()]
Param(
[Parameter(Mandatory=$true)]$devopsAccount,
[Parameter(Mandatory=$true)]$PersonalAccessToken,
[Parameter(Mandatory=$true)]$agentVmName,
...
do
{
  try
  {
    ...
    $latestReleaseDownloadUrl = "https://vstsagentpackage.azureedge.net/agent/2.148.2/vsts-agent-win-x64-2.148.2.zip"
...
$agentInstallationPath = Join-Path "C:" $agentVmName 
...
.\config.cmd --unattended --url $serverUrl --auth PAT --token $PersonalAccessToken --pool $PoolName --agent $agentVmName --runasservice
